...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True,
            encoding="utf-8", errors="replace", timeout=30, cwd=repo_root,
        )
        if result.returncode != 0:
...
def _cleanup_worktree(info: Dict[str, str] = None) -> None:
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True,
            encoding="utf-8", errors="replace", timeout=10, cwd=wt_path,
        )
    ...
    try:
        subprocess.run(
            ...
            capture_output=True, text=True,
            encoding="utf-8", errors="replace", timeout=15, cwd=repo_root,
        )
    except Exception as e:
    ...
    try:
        subprocess.run(
            ["git", "branch", "-D", branch],
            capture_output=True, text=True,
            encoding="utf-8", errors="replace", timeout=10, cwd=repo_root,
        )
    except Exception as e:
        ...
        if not force:
            ...
            try:
                result = subprocess.run(
                    ["git", "log", "--oneline", "HEAD", "--not", "--remotes"],
                    capture_output=True, text=True,
                    encoding="utf-8", errors="replace", timeout=5, cwd=str(entry),
                )
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    try:
        head_result = subprocess.run(
            ["git", "branch", "--show-current"],
            capture_output=True, text=True,
            encoding="utf-8", errors="replace", timeout=5, cwd=repo_root,
        )
